...
namespace osvr {
namespace util {
template<typename KeyType, typename ValueVariant>
class KVDataStore {
  public:
    ...
    /// Sets the value associated with the key. Overwrites any existing value stored at that key.
    template<typename T>
    ...
    /// Checks to see that a value has been associated with key, and furthermore that the value is of type T.
    template<typename T>
    bool contains(key_type const& key) const {
      ...
      // This signature of get() on variant returns nullptr if the contents are not of that type.
      T * contents = boost::get<T *>(it->second);
      return contents != nullptr;
    }
    
    /// Gets the value, of the type specified, identified by the key from the store.
    /// Assumes that contains<T>(key) is true: if it's not, it will return a default-constructed T.
    template<typename T>
    T get(key_type const& key) const {
      ...
      auto it = data_.find(key);
      assert(it != data_.end() && "Should be handled by the above check for contains");
